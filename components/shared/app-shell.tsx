...
import SchoolLogo from "@/assets/vidyalaya.svg";
...
export function AppShell({ roleSummary, schoolName, userLabel, children }: AppShellProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50">
      <div className={cn("mx-auto grid min-h-screen max-w-[1600px] transition-all duration-300 ease-in-out",
        isCollapsed
        ...
        <aside className={cn("fixed inset-y-0 left-0 z-50 flex flex-col border-slate-800/60 bg-slate-900 text-slate-300 shadow-xl transition-all duration-300 ease-in-out lg:sticky lg:top-1 lg:left-1 lg:h-[99dvh] rounded-xl lg:translate-x-0  lg:shadow-none", "w-[70vw] sm:w-[320px] sm:left-1 sm:h-[99dvh] sm:top-1", isCollapsed ? "lg:w-[65px]" : "lg:w-[256px] xl:w-[268px]",
          ...
          <div className="shrink-0">
            <div className={cn("inline-flex h-11 items-center border border-slate-800 bg-slate-950/40 font-medium uppercase text-slate-200 transition-all duration-300 rounded-t-xl overflow-hidden",
              isCollapsed ? "lg:w-full lg:justify-center px-0" : "w-full px-3 py-1.5 text-[11px]")}>
              <School className="h-4 w-4 shrink-0" />
              <span className={cn("ml-2.5 font-semibold tracking-wide transition-all duration-300 ease-in-out origin-left text-slate-100",
          ...
          {/* Nav Workspace: Takes all available middle space */}
          <nav className="flex-1 overflow-y-auto p-2 gap-1 content-start custom-scrollbar" aria-label="Primary navigation">
            {NAV_ITEMS.map((item) => {
              ...
              const linkStyles = cn("flex items-center h-10 transition-all duration-200 ease-in-out overflow-hidden whitespace-nowrap text-sm font-medium relative group border border-transparent border-b-slate-800/30 hover:rounded-xl", isCollapsed ? "lg:w-12 lg:justify-center px-0 mx-auto" : "w-full px-3.5",
                active
              ...
              return item.disabled ? (
                <span key={item.href} className={cn(linkStyles, "cursor-not-allowed opacity-40 text-slate-600 hover:bg-transparent")} aria-disabled="true" >
                  <Icon className="h-4 w-4 shrink-0" />
                  <span className={cn("ml-3 truncate transition-all duration-300 ease-in-out origin-left",
                    isCollapsed ? "lg:opacity-0 lg:w-0 lg:ml-0" : "opacity-100 w-auto"
                  )}>
                    {item.label}
                  </span>
                ...
                <Link key={item.href} href={item.href} title={isCollapsed ? item.label : undefined} className={linkStyles}>
                  <Icon className={cn("h-[18px] w-[18px] shrink-0 transition-transform duration-200 group-hover:scale-105",
          ...
          <div className="border-t rounded-b-xl border-slate-800/60 bg-slate-900 shrink-0 relative">
            <button type="button" onClick={() => setProfileOpen((prev) => !prev)} className={cn("flex items-center rounded-b-xl w-full text-slate-200 bg-slate-950/60 p-2.5 shadow-md transition-all duration-200 hover:bg-indigo-950/40 hover:border-indigo-500/50", isCollapsed ? "justify-center" : "gap-3")}>
              ...
              <div className={cn("flex-1 text-left min-w-0 transition-all duration-300", isCollapsed ? "hidden" : "block")}>
              ...
              <ChevronDown className={cn("h-4 w-4 text-slate-400 transition-transform duration-300 shrink-0",
                isCollapsed ? "hidden" : "",
            ...
            <div className={cn("absolute bottom-full left-2 right-2 z-50 mb-2 overflow-hidden rounded-xl border border-slate-800 bg-slate-950 shadow-2xl shadow-black/90 transition-all duration-200 ease-out origin-bottom",
              ...
              <Link href="/dashboard/profile" className="flex w-full items-center gap-3 px-4 py-3 text-sm text-slate-300 transition-colors hover:bg-slate-800 hover:text-white">
                <UserRound className="h-4 w-4 text-slate-400" />
                Profile Settings
              </Link>

              <form action={logoutAction}>
                <button type="submit" className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm text-rose-400 font-medium transition-colors hover:bg-rose-950/30 hover:text-rose-300 border-t border-slate-800/40" >
