  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
        <Card>
          <CardHeader><CardTitle>{canManageHostel ? "Add hostel" : "Hostel controls"}</CardTitle><p className="text-sm leading-6 text-slate-600">{canManageHostel ? "Create hostel buildings with warden and contact details." : accessCopy.summary}</p></CardHeader>
          <CardContent>{canManageHostel ? <HostelForm /> : <EmptyState title={accessCopy.title} description={accessCopy.description} />}</CardContent>
        </Card>
      ...
      <section className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
        <Card>
          ...
          <CardContent className="grid gap-5">
            {canManageHostel ? (<><HostelRoomForm hostels={hostels.map((h) => ({ id: h.id, code: h.code, name: h.name }))} /><div className="border-t border-slate-200 pt-5"><HostelAllocationForm students={students.map((s) => ({ id: s.id, name: s.fullName, meta: [s.class?.name, s.section?.name].filter(Boolean).join(" ") }))} hostels={hostels.map((h) => ({ id: h.id, code: h.code, name: h.name }))} rooms={rooms.map((r) => ({ id: r.id, hostelId: r.hostelId, roomNumber: `${r.hostel.code} - ${r.roomNumber}`, capacity: r.capacity, occupied: r.allocations.length, monthlyFee: r.monthlyFee?.toString() ?? null }))} /></div></>) : <EmptyState title={accessCopy.title} description={accessCopy.description} />}
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>{allocations.length ? (<TableFrame><Table><THead><tr><TH>Student</TH><TH>Hostel</TH><TH>Room</TH><TH>Fee</TH><TH>Consent</TH><TH>Status</TH></tr></THead><TBody>{allocations.map((allocation) => (<tr key={allocation.id}><TD>{allocation.student.fullName}</TD><TD>{allocation.hostel.code} - {allocation.hostel.name}</TD><TD>{allocation.room.roomNumber}{allocation.bedNumber ? ` / Bed ${allocation.bedNumber}` : ""}</TD><TD>{formatCurrency(Number(allocation.monthlyFee ?? allocation.room.monthlyFee ?? 0))}</TD><TD>{allocation.guardianConsent ? "Yes" : "Pending"}</TD><TD>{allocation.status}</TD></tr>))}</TBody></Table></TableFrame>) : <EmptyState title="No allocations" description="Allocated hostel students will appear here." />}</CardContent>
      </Card>
